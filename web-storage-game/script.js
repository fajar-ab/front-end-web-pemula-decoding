...
window.addEventListener("load", function () {
  if (typeof Storage !== undefined) {
    // * inisialisasi semua item web stirage yang kita gunakan jika ada

    // * untuk session storage
    //  nullis operator
    sessionStorage.setItem(
      sessionAnswerKey,
      sessionStorage.getItem(sessionAnswerKey) ?? ""
    );

    sessionStorage.setItem(
      sessionUserAttemptsKey,
      sessionStorage.getItem(sessionUserAttemptsKey) ?? 0
    );
    ...
    // * untuk locale storage
    localStorage.setItem(
      localTotalVictoryKey,
      localStorage.getItem(localTotalVictoryKey) ?? 0
    );

    localStorage.setItem(
      localMaximumAttemptsKey,
      localStorage.getItem(localMaximumAttemptsKey) ?? 0
    );
  } else {
